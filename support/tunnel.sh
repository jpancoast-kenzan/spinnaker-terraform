...
if [ "x$CLOUD_PROVIDER" == "x" ]; then
    echo "usage: $0 -c <cloud provider (aws only for now)> -a <start|stop>"
    exit 1
fi
...
echo "$SCRIPT_DIR/../$CLOUD_PROVIDER"
cd "$SCRIPT_DIR/../$CLOUD_PROVIDER"

ssh_user=$(terraform show | grep 'vars.ssh_user = ' | head -n1 | sed -e 's/.*vars.ssh_user\ \= //')
private_key_location=$(terraform show | grep 'vars.private_key = ' | head -n1 | sed -e 's/.*vars.private_key\ \= //')
bastion_public_ip=$(terraform show | grep 'vars.bastion_ip = ' | head -n1 | sed -e 's/.*vars.bastion_ip\ \= //')
...
if [ "$ACTION" == "start" ]; then
	...
	LOCAL_TUNNEL_COMMAND="$SCRIPT_DIR/spinnaker-tunnel-control.sh start"
    ...
    LOCAL_TUNNEL_COMMAND="$SCRIPT_DIR/spinnaker-tunnel-control.sh stop"
else
